...
class Node:
	...
	def update_neighbors(self, grid):
		self.neighbors = []
		# Check if node above the current node is a valid neighbor
		if self.row > 0 and not grid[self.row - 1][self.col].is_barrier():
			self.neighbors.append(grid[self.row - 1][self.col])

		# Check if node below the current node is a valid neighbor
...
def make_path(came_from, current_node, draw):
...
def algorithm(draw, grid, start, end):
	...
	# G_SCORE is the distance from start node to current node
	# H_SCORE (from h(point1, point2)) is the estimated distance to the end node
	# F_SCORE is the sum of G_SCORE & H_SCORE
	# The lowest F_SCORE path is the shortest path from start node to end node
